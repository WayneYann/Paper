...
set size 0.5,0.5
set xlabel '$X_{\rm O_2}$'
set ylabel 'Critical Strain Rate (1/s)'

#set label ''
p Hep.'data_Hep' u ($1):($3) w p ls 999 t '$Hep_exp$', \
  Hep.'data_Hep' u ($1):($5) w l ls 1 t '$Hep_cutoff$', \
  NB.'data_NB' u ($1):($3) w p ls 998 t '$NB_exp$', \
  NB.'data_NB' u ($1):($5) w l lt 2 t '$NB_cutoff$', \
  MB.'data_MB' u ($1):($3) w p ls 997 t '$MB_exp$', \
  MB.'data_MB' u ($1):($5) w l ls 3 t '$MB_cutoff$'
